...
class PartnerAddress(models.Model):
    ...
    job = fields.Char('职务')
    supervisor_id = fields.Many2one('partner.address','上级')
    gone = fields.Boolean('已离职')
    note = fields.Text('备注')
    same_office = fields.Boolean('同办公地址')
    ...
    @api.onchange('same_office')
    def onchange_same_office(self):
        for s in self:
            if s.same_office:
                s.province_id = s.partner_id.province_id
                s.city_id = s.partner_id.city_id
                s.detail_address = s.partner_id.office_address
