...
class ReportStockTransceive(models.Model):
    ...
    def get_context(self, sql_type='out', context=None):
        datetime_end = datetime.strptime(context.get('date_end'), "%Y-%m-%d") + timedelta(days=1)
        date_end = str(datetime.date(datetime_end))
    ...
    def find_source_move_line(self):
        ...
        return {
            'name': ('库存调拨' + 
                     datetime.datetime.strftime(date_start, "%Y-%m-%d") +
                      '~' + 
